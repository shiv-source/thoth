...
- purpose: antd Tabs (left rail, tabPlacement="start"; active pill via the .settings-tabs CSS rule + theme.ts Tabs tokens) over General/Doctor/Git — each tab one Card with icon'd SectionHeading sections, Dividers, and responsive two-column section grids: General (wiki path Input + model Select grouped by provider + shared API key, per-provider credentials section keyed off the models registry — base URL Input + write-only API key per provider with Configured/Not set Tags, save loading, saved/error Alerts); Doctor (pass Progress summary, Run checks, CheckRow status rows); Git (PAT Input.Password connect, account Avatar row + scope Tags, AutoComplete repo picker with public-repo guard, Switch auto-sync, Initialize & Push). Async state in the settings/doctor/git slices; tab rides the URL segment
