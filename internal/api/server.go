...
func newServer(d Deps) (*echo.Echo, *Hub) {
	...
	e.POST("/api"+APIVersion+"/capture", func(c echo.Context) error { return capture(c, d) })
	e.GET("/api"+APIVersion+"/capture/check", func(c echo.Context) error { return captureCheck(c, d) })
	e.GET("/api"+APIVersion+"/capture/inbox-count", func(c echo.Context) error { return captureInboxCount(c, d) })
	e.POST("/api"+APIVersion+"/capture/summarize", func(c echo.Context) error { return summarizeCapture(c, d) })
	e.GET("/api"+APIVersion+"/capture/read-later", func(c echo.Context) error { return captureReadLaterList(c, d) })
	e.DELETE("/api"+APIVersion+"/capture/read-later", func(c echo.Context) error { return captureReadLaterDelete(c, d) })
