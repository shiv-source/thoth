...
func NewGetTodos(fs agenttools.FS, path string) *GetTodos {
	if path == "" {
...
func NewUpdateTodos(fs agenttools.FS, path string) *UpdateTodos {
	if path == "" {
		path = DefaultTodosPath
	}
...
func (t *UpdateTodos) Run(ctx context.Context, args map[string]any) (string, error) {
	...
	content, err := agenttools.StringArg(args, "content")
	if err != nil {
		return "", err
	}
	rel, err := agenttools.CleanRel(t.path)
	if err != nil {
		return "", err
	}
...
// to inbox).
func NewGetInbox(fs agenttools.FS, dir string) *GetInbox {
	if dir == "" {
		dir = DefaultInboxDir
	}
...
// (defaults to inbox).
func NewFileInbox(fs agenttools.FS, dir string) *FileInbox {
	if dir == "" {
		dir = DefaultInboxDir
	}
...
func (t *FileInbox) Run(ctx context.Context, args map[string]any) (string, error) {
	...
	dest, err := agenttools.StringArg(args, "dest")
	if err != nil {
		return "", err
	}
	...
	src, err := agenttools.CleanRel(filepath.Join(t.dir, path))
	if err != nil {
		return "", err
	}
	dst, err := agenttools.CleanRel(filepath.Join(dest, path))
	if err != nil {
		return "", err
	}
...
func NewRemember(fs agenttools.FS, path string, now func() time.Time) *Remember {
	if path == "" {
...
func (t *Remember) Run(ctx context.Context, args map[string]any) (string, error) {
	...
	fact, err := agenttools.StringArg(args, "fact")
	if err != nil {
		return "", err
	}
	rel, err := agenttools.CleanRel(t.path)
	...
	case errors.Is(err, fs.ErrNotExist):
		...
		meta := wiki.NoteMeta{Title: "Memory", Date: t.now().Format("2006-01-02")}
		data = wiki.FormatNote(meta, "")
	default:
