...
# Wired into .husky/pre-commit. Enforces CLAUDE.md § Repo rules ("never
...
if [ "$branch" = "main" ]; then
  echo "main-guard: refusing commit on main — changes live on branches (CLAUDE.md § Repo rules): git switch -c <type>/<scope>/<slug>" >&2
  exit 1
fi
