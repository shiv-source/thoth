...
- `ParseNote(content)` — block-parses the frontmatter fence (tolerates CRLF, a UTF-8 BOM, and the `...` terminator; only a column-0 `---`/`...` line closes), requires `title`, accepts `kind:` as an alias for `type:`, validates `date`/`type`/`tags`, and returns `NoteMeta` + body
