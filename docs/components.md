...
`Client` talks to api.github.com (identity + repos; the token is never sent anywhere else) and `Repo` stores the PAT and identity in the `github_auth` table; `Service` is a thin bundle of the two for API wiring. The git sync itself (init, remote, commit, push — errors sanitized) lives in `internal/api/v1/git.go`, run on the same pure-Go `agent/git` backend the agent's git tools use, so no git binary is needed anywhere. Details: [Schema](schema.md).
