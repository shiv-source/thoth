...
- `ParseNote(content)` — block-parses the frontmatter fence (tolerates CRLF, a UTF-8 BOM, and the `...` terminator; only a column-0 `---`/`...` line closes), requires `title`, accepts `kind:` as an alias for `type:`, validates `date`/`type`/`tags`, reads `source:` (the capture provenance), and returns `NoteMeta` + body
...
- `Bookmark` / `AddReadLater` — the link-file writers: append one rulebook-format line (`- [title](url) — reason`) to `links/bookmarks.md` (grouped under `## category`) or the flat `links/read-later.md` queue. The file (with note frontmatter, so the master list is a valid, searchable note) is created when missing, the write is atomic, the read-modify-write is mutex-serialized, and a URL already saved returns `ErrDuplicateBookmark`. Titles and URLs are escaped into the markdown line (percent-encoding, backslash escapes) so any input — a `)`-laden Wikipedia URL, a `[bracketed]` title — round-trips through the parser; dedup compares `NormalizeURL`d URLs (case, default ports, fragments) so accidental duplicates collapse while the stored link stays the exact URL you gave. `Bookmarks()` / `ReadLater()` parse the files back for the dedup check and listing; `RemoveReadLater(url)` clears one URL from the queue (idempotent, normalized match) for the dashboard's read-later triage
- `Save(o)` — the promotion/save path shared by the API and capture: `ValidFolder` + `ValidSourceURL`, a kebab-case filename (date-prefixed in `meetings/`/`daily/`), `FormatNote` content checked by `Validate`, atomic write. A slug collision never silently overwrites — the next free `-2`/`-3` suffix is used (serialized by a per-wiki save mutex), so two captures with the same title become `note.md` and `note-2.md`
- `ValidSourceURL(u)` — the shared capture gate: an absolute `http(s)` URL (or empty), used by `Save` (via `SaveOptions.SourceURL` → `source:` frontmatter) and the capture endpoints
...
- `CountByPrefix(prefix)` — how many indexed notes live under `prefix/`; the capture `inbox-count` endpoint (the extension's toolbar badge) is this one query
...
`Deps` carries every dependency; `New(d Deps) *echo.Echo` wires all routes. `Hub` owns the WebSocket lifecycle: one active turn per conversation, supersede-on-send, cancel, and a 500-message replay buffer for reconnects. It also keeps a client registry and `Broadcast`s server-push frames (non-blocking; slow clients are skipped). Clients still send `presence` frames for their tab's visibility, but Thoth Agent keeps no idle processes to flush, so they are accepted for protocol compatibility and ignored server-side. When `Deps.Events` carries the event bus (`go-warehouse/events`, built in `internal/cli/serve.go`), `newServer` subscribes and forwards every `wiki.Changed` batch to all clients as a `wiki_changed` frame, and every `sync.Result` (a completed scheduled push) as a `sync_result` frame the UI surfaces as a notification. `capture.go` owns the unified capture surface — `POST /api/v1/capture` (bookmark/note/selection/readlater), `GET /api/v1/capture/check` (dedup), `GET /api/v1/capture/inbox-count` (badge), `GET`/`DELETE /api/v1/capture/read-later` (the dashboard's read-later triage), and `POST /api/v1/capture/summarize` (agent → `knowledge/` note) — the single write path the browser extension and the dashboard quick capture share. Protocol details: [API](api.md).

## internal/store

Conversations, messages, the `providers` table, the llm_models registry, and the sync tables (`sync_providers`, `sync_connections`, `sync_push_history`) in the same `thoth.db` (separate `*sql.DB`, WAL makes sharing safe). The whole schema lives in embedded `.sql` migrations in `migrations/`, applied in filename order and gated on `PRAGMA user_version`; a single-row `app_metadata` table (enforced by `CHECK (id = 1)`) holds install facts (`installation_id`, `created_at`, seeded by `EnsureMetadata` on boot). IDs are valid RFC 4122 v4 UUIDs (`google/uuid`); the conversation id is the chat history key passed to the agent as `sessionID`. Timestamps are stored UTC so ordering is chronological.
...
`syncproviders.go` owns the sync provider catalog (`ListSyncProviders` with connection counts, `SyncProvider`/`SyncProviderBySlug`, `CreateSyncProvider`, `EnsureSyncProvider` — the `INSERT OR IGNORE` seed, `UpdateSyncProvider`, `DeleteSyncProvider` with `ErrSyncProviderInUse` while connections exist, `ErrSyncProviderProtected` for the seeded local row). `connections.go` owns `sync_connections` (`ListConnections`, `Connection`, `CreateConnection`, `UpdateConnection`, `DeleteConnection` with `ErrConnectionProtected`, `SetConnectionSyncResult`, `EnsureLocalBackup` — the idempotent protected local connection). `pushhistory.go` owns `sync_push_history` (`AppendPushHistory` prunes to 20 rows per connection, `ListPushHistory` newest-first); `SetConnectionSyncResult` appends to it, so every outcome lands in the history. Details: [Schema](schema.md).

## internal/sync — the multi-provider sync engine

Owns the driver seam the API and the agent-tool wiring depend on, never a concrete provider. `Driver` (in `driver.go`) is a small stateless interface — `Kind`, `Fields` (the descriptor-driven connect form inputs), `Verify` (returns the token-free identity), `Targets` (repos for git kinds), `Push` — implemented by `gitDriver` (github + gitlab over `agent/git`), `s3Driver`, and `localDriver`. `Restorer` is the optional restore capability: drivers that store archives (s3, local) implement it (`Snapshots` lists restore points, `Restore` returns the archive as a `ReaderAt` + size for `wiki.ImportFrom` — the local driver requires a plain basename that resolves inside the backup folder, blocking path traversal); git-kind connections are push-only. `registry.go` builds the `Service` (the store + driver registry handle the API depends on), whose `Push` centralizes the push path — resolve driver, decode config/identity, `pushWithRetry` (retries only `ErrRetryable` network/server-fault failures with backoff), record `SetConnectionSyncResult`. `s3.go` honors the `snapshot` mode (`stable` wiki.zip default · `history` timestamped keys · `both`) and an optional `retention` (prunes the oldest history keys, prefix-aware). `scheduler.go` is the auto-sync: a background loop that pushes enabled connections whose `interval` (minutes) has elapsed since their last attempt (so a failing connection cools down), skipping any connection already in flight, reporting each outcome as a `Result` for the serve layer to publish on the event bus. Details: [API](api.md), [Schema](schema.md).
...
## browser-ext — the capture browser extension

A standalone `browser-ext/` pnpm workspace member (Manifest V3, Chrome + Firefox from one shared core) that captures bookmarks, selections, and quick notes into the same wiki the dashboard reads — a second consumer of the REST API, not a second save rule. The popup is **React + antd** (the same stack as the dashboard, with a compact mirror of its theme in `src/popup/theme.tsx`); the background worker and core logic stay framework-free. The build (`make ext-build`, esbuild with the automatic JSX runtime) emits loadable unpacked packages into `browser-ext/dist/chrome` and `browser-ext/dist/firefox`.

- `src/core` — the browser-agnostic logic, all dependency-injected for tests: `api.ts` (the `ThothClient` REST client + `ThothError`, which carries the 409 "already saved" path), `server.ts` (probes `127.0.0.1:8333` then `8334`), `config.ts` (`resolveBaseUrl`/`saveBaseUrl` over a `StorageLike` seam), `format.ts` (selection → blockquote body, `selectionTitle` (derives a title from the quote's first sentence), `sourceTag` (the source-domain capture tag), `draftToCapture`, `openNoteUrl`), `menus.ts` (the four context menus + `draftForMenu`), `badge.ts` (`refreshBadge` = the inbox count), `background.ts` (`initBackground` wiring), `storage.ts` (the pending capture draft), and `webext.ts` (the `BrowserAPI` seam — `browser.*` on Firefox, `chrome.*` on Chrome)
- `src/chrome` / `src/firefox` — thin manifests (`service_worker` vs `background.scripts` + a Gecko id) and a one-line entry each
- `src/popup` — React draft form (`PopupApp` in `App.tsx`, mounted by `popup.tsx` under the popup `ConfigProvider` theme): server status ("start thoth" state on connect failure), connection config, kind tabs (note/bookmark/read-later/summarize), editable fields, dedup ("already saved → open it"), and "Open in Thoth" after a save
- `e2e` — the Playwright suite: `mock-server.mjs` (a Node dev server implementing the capture API contract, recording every POST) and `capture.spec.ts`, which loads the built extension into a persistent Chromium context and drives the popup end to end (quick note, bookmark, dedup, read-later, validation) via `make ext-e2e`

The capture flow: a context-menu click writes a draft to storage and opens the popup prefilled (feature #3) — full-page text is only ever captured on the explicit summarize action or the popup's "include full page text" toggle (feature #7). The summarize menu grabs the page text up front via `scripting.executeScript` under the `activeTab` grant and posts `POST /api/v1/capture/summarize`. All writes go through the server's capture endpoints, so validation stays in one place (the wiki contract), never reimplemented in the extension.

## internal/api — sync

The `/api/v1/sync/*` routes (providers CRUD, connections CRUD + targets/push/active, and the restore flow: `snapshots` list + `restore`) are the host's multi-provider wiki sync, driven entirely by the `internal/sync` drivers. `pushConnection` runs the shared `Service.Push` path (retry-with-backoff, result recording); `restoreConnection` downloads a stored archive and runs the same backup-first `wiki.ImportFrom` merge as the upload path, then reindexes and pushes a `wiki_changed` frame. Every connection DTO carries `push_history` (the recent runs). Every step reports a fixed sanitized error that never echoes credentials. Details: [API](api.md), [Security](security.md).
