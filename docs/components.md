...
Owns the driver seam the API and the agent-tool wiring depend on, never a concrete provider. `Driver` (in `driver.go`) is a small stateless interface — `Kind`, `Fields` (the descriptor-driven connect form inputs), `Verify` (returns the token-free identity), `Targets` (repos for git kinds), `Push` — implemented by `gitDriver` (github + gitlab over `agent/git`), `s3Driver`, and `localDriver`. `Restorer` is the optional restore capability: drivers that store archives (s3, local) implement it (`Snapshots` lists restore points, `Restore` returns the archive as a `ReaderAt` + size for `wiki.ImportFrom`); git-kind connections are push-only. `registry.go` builds the `Service` (the store + driver registry handle the API depends on), whose `Push` centralizes the push path — resolve driver, decode config/identity, `pushWithRetry` (retries only `ErrRetryable` network/server-fault failures with backoff), record `SetConnectionSyncResult`. `s3.go` honors the `snapshot` mode (`stable` wiki.zip default · `history` timestamped keys · `both`) and an optional `retention` (prunes the oldest history keys). `scheduler.go` is the auto-sync: a background loop that pushes enabled connections whose `interval` (minutes) has elapsed, reporting each outcome as a `Result` for the serve layer to publish on the event bus. Details: [API](api.md), [Schema](schema.md).
