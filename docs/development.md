...
The frontend lives in a **pnpm workspace** at the repo root (`pnpm-workspace.yaml` registers `web/`, `docs-site/`, and `browser-ext/`): dependencies install and lock at the root (`pnpm-lock.yaml`), and the root scripts proxy into the web app — `pnpm dev` / `pnpm test` / `pnpm typecheck` / `pnpm lint` / `pnpm format` work from anywhere. `.npmrc` pins `save-exact`. The docs site (Docusaurus) runs through `pnpm --filter thoth-docs <cmd>` (`start`/`build`), or `make docs-dev` / `make docs-build`. The browser extension (MV3, Chrome + Firefox) is its own workspace member `browser-ext/` — React + antd popup over a framework-free TypeScript core — built with `make ext-build` (esbuild emits loadable unpacked packages into `browser-ext/dist/{chrome,firefox}`) and quality-gated by `make ext-check`.
...
| `make build [VERSION=…]` | Release binary with version stamping (`-buildvcs=false` keeps builds reproducible across toolchains; the version still stamps via `-ldflags -X`) |
...
| `make ext-build` | Build the unpacked Chrome + Firefox extension packages into `browser-ext/dist/` |
| `make ext-check` / `make ext-e2e` | Extension gates (`ext-lint` · `ext-typecheck` · `ext-test` · `ext-build`) and the Playwright e2e (part of `make check`) |
...
6. Browser extension: `make ext-check` (`pnpm --filter thoth-ext lint|typecheck|test|build`) and `make ext-e2e` (Playwright: loads the built extension into Chromium and drives the popup against the mock capture server)
7. Tooling JS: `node --test .github/actions/issue-labels/test/*.test.mjs` (the issue-labels action suite; also a CI job)
