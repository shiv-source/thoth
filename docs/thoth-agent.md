...
| **Write access** | The CLI ran with `--dangerously-skip-permissions` (fully unattended headless) | A full FS-backed tool catalog bounded to the wiki by `SafePath`: `read_file`/`write_file`/`list`/`list_tree`/`grep`, note tools (`write_note`/`read_note`), file mutation (`edit_file`/`append_file`/`rename_file`/`delete_file`), discovery (`list_recent`/`search_by_tag`), wiki ops (`get_todos`/`update_todos`/`get_inbox`/`file_inbox`/`remember`), `get_time`, and `search` over the FTS index |
