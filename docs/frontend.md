...
| `SettingsModal` | Tabbed: **General** (the 6 config fields + save, toasts on success/error), **Doctor** (shared check suite via `GET /api/doctor`, green ✓ / red ✗ rows, "Run checks"), **Git remote** (URL field, Save persists `git_remote_url`, "Initialize & Push" calls `POST /api/git/setup`); backdrop, ✕, and Esc close |
