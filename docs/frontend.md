...
| `NoteViewer` | Inline note reader filling the content area (Esc or ✕ closes — the open note is the URL, not an overlay): note content from the note slice (stale-path responses discarded), Skeleton loading, Alert errors, Copy raw. Only `.md`/`.markdown` paths (case-insensitive, matching `wiki.IsMarkdownPath`) are fetched and rendered as Markdown — any other file type (images, PDFs, attachments, …) gets a "can't be previewed" state instead of raw bytes |
