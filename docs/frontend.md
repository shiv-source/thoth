...
│   ├── notes/           #   NotesPage
│   ├── search/          #   SearchPage
│   ├── settings/        #   SettingsPage dispatcher → Settings{General,Providers,Sync,Doctor}Page
...
| `SettingsPage` | Segment dispatcher: renders one of four route-level sub-pages (`/settings/<section>`) — `SettingsGeneralPage`, `SettingsProvidersPage`, `SettingsSyncPage`, `SettingsDoctorPage` — sharing `SettingsShell` (header + a left rail `Menu` on a bordered surface card; the active item gets a primary pill via the `.settings-menu` CSS rule; the section rides the URL `/settings/<section>`; the rail stays fixed while the content scrolls in a shared `max-w-4xl` column). Every section is one Card whose fields are grouped into enterprise sections by the shared `FormSection` (icon heading + one-line description + fields) with hairline dividers between them, single inputs capped at `max-w-lg` and pairs in a 2-col grid. The General sub-page owns a `Form` via `useSettingsForm` (fetch + seed + `save` merging only its own fields through `settingsBody`, so the PUT payload is always complete) and shares the `SaveFooter` save bar; its sections are **Knowledge base** (`WikiPathInput` with the `DirBrowserModal` picker + the scaffold folders tag `Select`), **Default model & context** (Provider/Model cascade `Select` pair — the Provider select is view state, the Model select the saved form field — + the context-injection `Switch`), **Chat history** (the `conversation_retention_days` auto-delete `InputNumber`, `min=0`, 0 keeps everything), and **Backup & transfer** (Export / Export-with-history / Import buttons with a progress bar). The other sections: **Providers** (an Add-provider `Button` + a `Collapse` panel per provider from the `/api/v1/providers` list; the header carries name + model-count/key/endpoint `Tag`s and Edit/Delete icon actions, the body holds the provider's models `Table` with colored tag chips and an `Empty` state; the `ProviderModal` adds/edits name + base URL + write-only API key + custom request headers as key/value rows (Portkey-style gateway headers), the `ModelModal` (combobox tag `AutoComplete` + provider `Select` pre-filled from the panel) adds/edits models, per-row `Popconfirm` delete; mutations refetch the providers list and the grouped registry), **Doctor** (pass `Progress` summary, Run checks button, `CheckRow` status rows), **Sync** (`SyncConnectionCard` per connection: identity line, provider-driven editable config fields, enabled `Switch`, push/disconnect/set-active actions, a Restore modal for s3/local connections backed by `GET /api/v1/sync/connections/:id/snapshots` + `POST …/restore`, and a compact recent-run history; `SyncConnectForm` renders the connect form from the provider's field descriptors in a full-width panel; `SyncProviderEditor` shows the always-visible provider catalog as a bordered list plus an add-provider panel) — all async state in the sync/settings/doctor slices |
...
| `FormSection` | The enterprise grouping block shared by the settings Cards: an icon'd `SectionHeading` + optional one-line description + the fields below, with `Divider`s between sections — every settings tab uses it, so the four pages share one vertical rhythm |
...
| `DashboardPage` | Landing: greeting header, four KPI `StatTile`s, quick-action `Button`s, the **Overview** widgets (`QuickCaptureCard`, `ContinueCard`, `NeedsAttentionCard`, `TodayCard`, `RecentNotesCard`, `TagsCard`, `TodosCard` — a checkable mock checklist, `WikiStorageCard`) and the **Insights** rows (`ChartCard` wrapping four Chart.js charts, blue palette), sections separated by the shared `SectionHeader` kicker. Mock data lives in `pages/dashboard/dashboardMock.tsx`, tagged with its issue until the index endpoints land |
...
| `QuickCaptureCard` | Overview "Quick capture": antd `Space.Compact` single-line capture input + primary button (Enter or click) that files into `inbox/` through the unified capture endpoint (`POST /api/v1/capture` — the same save path the browser extension uses), toasts the saved path, and hands it to the page so it opens the note |
| `ReadLaterCard` | Overview "Read later": the `links/read-later.md` queue listed from `GET /api/v1/capture/read-later`, each row openable (external link) with **Bookmark** (promote to `links/bookmarks.md` and clear) and **Done** (clear) triage actions via `POST /api/v1/capture` + `DELETE /api/v1/capture/read-later` |
