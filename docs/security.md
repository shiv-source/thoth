...
- Sync credentials (git PATs, S3 access keys) are stored plaintext in the `config` JSON of `sync_connections` rows in `thoth.db` — the same trust model as the `gh` CLI's own credentials file; the API never returns them (secret fields surface as `has_<key>` booleans) and errors never echo them, and they are only ever sent to the configured destination during a push. The push (`POST /api/v1/sync/connections/:id/push`) runs on the pure-Go `agent/git` backend (no git binary) and authenticates with the stored token as BasicAuth. SSH remotes and credential-helper/SSH-agent flows are not honored: a sync requires the stored token/access key. Errors are sanitized fixed messages that never include the remote URL, bucket, or token, and transient failures are retried with backoff only (a permanent credential error is surfaced immediately)
- The API keys are stored plaintext in the `providers` table in thoth.db — one row per provider, the same local-file trust model as the PAT; Thoth Agent sends them only to the model provider's endpoint on this machine, and the API never returns them (`has_api_key` only, and a `PUT` with an empty key leaves the stored key unchanged). Migration `0011` moved the keys here from the legacy `provider_<slug>_api_key` settings keys
