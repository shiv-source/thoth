...
- The API key is stored plaintext in the `settings` table (`api_key`) — same local-file trust model as the PAT; it is injected as `ANTHROPIC_API_KEY` only into spawned Claude CLI processes on this machine, and the API never returns it (`has_api_key` only, and `PUT` with an empty key leaves it unchanged)
