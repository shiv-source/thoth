...
- **Per-provider credentials** — each provider can have its own API key and base URL (`provider_<slug>_api_key` / `provider_<slug>_base_url`), falling back to the shared key and the provider's default endpoint. Settings → Providers exposes them.
