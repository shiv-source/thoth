...
| Capture | `POST /api/v1/capture` is the unified write surface for the browser extension and the dashboard quick capture. The body is `{kind, url, title, text, reason, tags, folder, category}` where `kind` is `bookmark` \| `note` \| `selection` \| `readlater`:
  - **bookmark** — appends one line to `links/bookmarks.md` (grouped under `## category`, the rulebook's master-list format) via `wiki.Bookmark`. Metadata only — full-page text is never captured implicitly. `409 {error, path}` when the URL is already saved.
  - **note / selection** — files the text via `wiki.Save` with `source:` frontmatter (the capture's URL) so provenance survives and dedup is possible.
  - **readlater** — appends to `links/read-later.md` (flat queue).
  Returns `{path, title, type}` (201). |
| Capture dedup | `GET /api/v1/capture/check?url=…` answers the cheap "is this URL already in `links/bookmarks.md`?" check the extension runs before bookmarking: `{exists: bool, path?}` with the existing file path when saved, so the UI can offer "already saved → open it". |
...
| Read-later triage | `GET /api/v1/capture/read-later` lists the queue (`{items:[{title,url,reason}]}` from `links/read-later.md`); `DELETE /api/v1/capture/read-later?url=…` removes one URL (idempotent). The dashboard's **Read later** card triages the queue — open, promote to a bookmark, or clear an item. |
| Capture summarize | `POST /api/v1/capture/summarize` (`{url, title, text}`) runs one assistant turn over the captured page text and promotes the summary to a `knowledge/` note with `source:` in frontmatter — the extension's "Ask Thoth to summarize this page". 502 when the assistant produces no summary. |
