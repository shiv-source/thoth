...
| `GET /api/notes?path=` | wiki-relative markdown path | `{path, content}` — 400 when the path is missing, escapes the wiki root, or is not a markdown note (`.md`/`.markdown`, per `wiki.IsMarkdownPath`); 404 when the note doesn't exist |
