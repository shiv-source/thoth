...
|---|---|---|
| `GET /api/health` | — | `{status, backend:{name, api_key_configured, model, provider}, wiki:{path,exists}, version, dev, commit, default_wiki_path}` — `backend` is the native agent (name `thoth-agent`; `api_key_configured` is whether a key is stored, the key itself is never returned; `provider` comes from the selected model's `llm_models` row); `dev` is true under `serve --dev` (the UI shows a warning banner); `commit` is the full git commit id the dev server runs from (empty otherwise); `default_wiki_path` is the mode's wiki default in tilde form (`~/.thoth/wiki`, or `~/.thoth/dev/wiki` in dev) — the settings hint reads it |
