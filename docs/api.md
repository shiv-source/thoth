...
| Capture summarize | `POST /api/v1/capture/summarize` (`{url, title, text}`) runs one assistant turn over the captured page text (bounded at 500 KB) and promotes the summary to a `knowledge/` note with `source:` in frontmatter — the extension's "Ask Thoth to summarize this page". 502 when the assistant produces no summary; 504 when the turn exceeds its 90 s timeout. |
