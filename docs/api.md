...
| Auto-sync | A background scheduler (started in `serve`) pushes enabled connections whose `interval` config field (minutes) has elapsed since their last successful sync. Each result is broadcast to connected clients as a `sync_result` WS frame, which the UI surfaces as a notification. |
