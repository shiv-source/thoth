...
| Capture | `POST /api/v1/capture` is the unified write surface for the browser extension and the dashboard quick capture. The body is `{kind, url, title, text, reason, tags, folder, category}` where `kind` is `bookmark` \| `note` \| `selection` \| `readlater`:
  - **bookmark** — appends one line to `links/bookmarks.md` (grouped under `## category`, the rulebook's master-list format) via `wiki.Bookmark`. Metadata only — full-page text is never captured implicitly. `409 {error, path}` when the URL is already saved. Titles, categories and reasons must be single lines.
  - **note / selection** — files the text via `wiki.Save` with `source:` frontmatter (the capture's URL) so provenance survives and dedup is possible. Bodies are bounded at 500 KB (400 when larger); a slug collision (`hello.md` already exists) files as `hello-2.md` rather than overwriting.
  - **readlater** — appends to `links/read-later.md` (flat queue).
  Returns `{path, title, type}` (201). |
| Capture dedup | `GET /api/v1/capture/check?url=…` answers the cheap "is this URL already in `links/bookmarks.md`?" check the extension runs before bookmarking: `{exists: bool, path?}` with the existing file path when saved, so the UI can offer "already saved → open it". URL comparison is normalized (scheme/host case, default ports, fragments) so accidental duplicates collapse; the stored link is always the exact URL you gave. |
| Capture inbox count | `GET /api/v1/capture/inbox-count` returns `{count}` — the number of unfiled notes under `inbox/`, read from the index. The extension's toolbar badge (#4) is this one query. |
| Read-later triage | `GET /api/v1/capture/read-later` lists the queue (`{items:[{title,url,reason}]}` from `links/read-later.md`); `DELETE /api/v1/capture/read-later?url=…` removes one URL (idempotent, normalized match). The dashboard's **Read later** card triages the queue — open, promote to a bookmark, or clear an item. |
| Capture summarize | `POST /api/v1/capture/summarize` (`{url, title, text, tags?}`) runs one assistant turn over the captured page text (bounded at 500 KB) and promotes the summary to a `knowledge/` note with `source:` in frontmatter — the extension's "Ask Thoth to summarize this page". The optional `tags` become the note's frontmatter tags (the extension sends the source-domain tag). 502 when the assistant produces no summary; 504 when the turn exceeds its 90 s timeout. |
