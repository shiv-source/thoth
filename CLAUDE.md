...
├── web/                  # React 19 + TS strict + Tailwind v4 + pnpm
│   └── src/{api,ws,hooks,components}/
├── .github/workflows/    # CI: vet → race → 80% coverage gate → lint → 5 cross-compiles → frontend
├── Makefile              # web | build | run | test | race | lint | clean
└── docs/superpowers/     # untracked working docs (spec + plan) — never commit
```
...
- `docs/superpowers/` is untracked (working docs only) — never `git add` it.
- Generated/ignored: `bin/`, `web/dist/`, `internal/webui/dist/`, `web/node_modules/`, `*.db`.

## Runtime data

`~/.thoth/`: `config.toml`, `thoth.db`, `wiki/` (default). Localhost-only, no auth. `thoth doctor` diagnoses the setup; `--fix` repairs config/wiki/index only.
